...
# Step 4 in Similarity-Based Chained Transfer Learning algorithm
# Transfer learning with the forecasting set of all other meters and the transfer path
# The structures and weights of the algorithm, the csv report are saved for transfer learning
...
### model_1:   (new) The source meter from the source-target meter pair
### model_2:   (new) The target meter from the source-target meter pair
...
def SBCTL(seed, model_1, model_2, cuda, cell_type, window_source_size, window_target_size, epochs, batch_size, hs, to_plot,
    ...
    def generate_windows(data):
        ...
        for idx in idxs:
            x_train.append(
                train_source[idx:idx + WINDOW_SOURCE_SIZE].reshape((1, WINDOW_SOURCE_SIZE, train_source.shape[1])))
            y_usage_train.append(
                train_source[idx + WINDOW_SOURCE_SIZE:idx + WINDOW_SOURCE_SIZE + WINDOW_TARGET_SIZE, -1].reshape(
                    (1, WINDOW_TARGET_SIZE, 1)))
        ...
        for idx in idxs:
            x_test.append(
                test_source[idx:idx + WINDOW_SOURCE_SIZE].reshape((1, WINDOW_SOURCE_SIZE, test_source.shape[1])))
            y_usage_test.append(
                test_source[idx + WINDOW_SOURCE_SIZE:idx + WINDOW_SOURCE_SIZE + WINDOW_TARGET_SIZE, -1].reshape(
                    (1, WINDOW_TARGET_SIZE, 1)))
    ...
    # Before use, pick the best model from initial model and save as"meter_{}_transfer_model.pt", in "transfer_models" (the following) dir.

    os.chdir('/Users/farewell/Desktop/forecasting_models')

    model.load_state_dict(torch.load("forecasting_model_meter_{}.pt".format(model_1)))

    print("Model {}'s loaded...".format(model_1))
    ...
    print("Model 2's state_dict:")
    for param_tensor in model.state_dict():
        print(param_tensor, "\t", model.state_dict()[param_tensor].size())

    os.chdir('/Users/farewell/Desktop/forecasting_models')

    torch.save(model.state_dict(), "forecasting_model_meter_{}.pt".format(model_2))
    ...
    if create_file:
        ...
        anom_data = pd.read_csv("{}.csv".format(model_2))
        ...
        output = anom_data[
            ['DT_TIMESTAMP', 'year', 'month_of_yr', 'day_of_yr', 'day_of_month', 'day_of_week', 'weekend', 'holiday',
             'hour', 'minute',
        ...
        output.to_csv(model_2 + "_MODEL_all.csv", sep=',', index=False)

        if just_anoms_file:
            output = output.loc[output['5p anomaly'] == 1]
            output.to_csv(model_2 + "_MODEL_anoms.csv", sep=',', index=False)
            print("Anomaly file saved as {}".format(model_2 + "_MODEL_anoms.csv"))

        print("All Data saved as {}".format(model_2 + "_MODEL_all.csv"))

        os.chdir('/Users/farewell/Desktop/forecasting_set')

    # return accuracy results, time, training and testing loss, and plotting arrays

    return mae3, mape3, model_1, model_2, total / 60.0, train_loss, test_loss, for_plotting
...
# INPUTS:
#	seed:		In SBCTL only one seed needed.
### model_1:   (new) The source meter from the source-target meter pair
### model_2:   (new) The target meter from the source-target meter pair
#   cuda:       2 options, either True or False. If True use GPU, if False use CPU
#   cell_type:  3 options, either 'rnn', 'gru', or 'lstm' -- this just chooses which cell to use in the model
...
#   window_target_size: Can be any reasonable number. Correlates to number of timesteps to use in prediction function. This is N, from paper.
#                       This is how many timesteps you would like to predict ahead.
#	epochs:		In SBCTL, only less than 3 epochs are needed.
#   batch_size: Can be any reasonable number. This tells the model how many random samples to use at once. The higher this number is, the
#               faster training is, but the worse accuracy is (very slightly). The smaller it is, the slower training is, but better accuracy
#               (again very slightly). Recommend using a power of 2 as the batch size -- hence 32, 64, 128, or 256.
#   hs:         Can be any reasonable number. This tells the model how many hidden units to use in the respective cells. Try using powers of 2
...
path = pd.read_csv('SBCTL_path.csv')

# Iterate through the files in forecasting set
os.chdir('/Users/farewell/Desktop/forecasting_set')
transfer_model_all = []
for index, row in path.iterrows():
    ...
    # Build model for each meter following the transfer learning path
    d = np.loadtxt("./{}.csv".format(model_2), delimiter=",", skiprows=1, dtype=str)
    print("Loading 2ND dataset {}...".format(model_2))



    # for one dataset at a time:
    if __name__ == "__main__":
        mae, mape,  model_1, model_2, total_mins, train_loss, test_loss, for_plotting = SBCTL(seed=0,  model_1 = model_1, model_2 = model_2,
                                                                          cuda=False, cell_type='lstm',
                                                                          window_source_size=12, window_target_size=6,
                                                                          epochs=3,
                                                                          batch_size=256, hs=64, to_plot=False,
                                                                          threshold_1=0.05, threshold_2=0.10,
                                                                          threshold_3=0.25, threshold_4=0.50,
